...
def startGlobalListener():
    """
    Global hotkeys inside Python were previously implemented using the
    `keyboard`/`pynput` libraries. On Linux they require root access and
    are unreliable on Wayland compositors like Hyprland.

    This fork intentionally disables in-process global hotkeys. Use Hyprland
    keybinds to call the app's IPC commands instead.
    """
    logger.info("Global hotkeys are disabled in this uinput-only Wayland build (use Hyprland binds + IPC).")
    return False

def stopGlobalListener():
    """No-op (see startGlobalListener)."""
    return
...
def dispatchIpcCommand(cmd: str):
    """Handle a command coming from the IPC server.

    This runs on the Tk main thread (scheduled via app.after).
    """
    app = getApp()
    if not app:
        return
    ...
    # Allow Hyprland binds to switch tabs.
    ...
    # Active tab routing
    current = getattr(app, "currentPage", 0)

    try:
        if current == 1:
            from modules.functions import drumsMacroFunctions as _drums
            _route = _drums
        elif current == 3:
            from modules.functions import midiToQWERTYFunctions as _m2q
            _route = _m2q
        else:
            from modules.functions import midiPlayerFunctions as _midi
            _route = _midi
    except Exception:
        return

    # Common controls (map to whichever module has them)
    mapping = {
        "play": ["playButton", "play", "startPlayback"],
        "pause": ["pausePlayback", "pause", "togglePause"],
        "stop": ["stopPlayback", "stop"],
        "slow_down": ["slowDownPlayback", "slowDown"],
        "speed_up": ["speedUpPlayback", "speedUp"],
    }
    ...
    for name in candidates:
        fn = getattr(_route, name, None)
        if callable(fn):
            try:
                fn()
            except TypeError:
                # Some functions might expect args; ignore.
                try:
                    fn(None)
                except Exception:
                    pass
...
def scheduleIpcCommand(cmd: str):
    """Thread-safe: can be called from any thread."""
    app = getApp()
    if not app:
        return
    try:
        app.after(0, lambda: dispatchIpcCommand(cmd))
    except Exception:
        pass
